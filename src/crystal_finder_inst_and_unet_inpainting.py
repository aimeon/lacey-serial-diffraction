...
def inpaint_with_mask_skimage(img, mask_bool):
    # img float [0,1], mask_bool True where grid is
    if hasattr(img, "numpy"):
        img = img.numpy()
    img = np.asarray(img, dtype=np.float32)

    if hasattr(mask_bool, "numpy"):
        mask_bool = mask_bool.numpy()
    ...
    # biharmonic expects float image; mask True means “inpaint here”
    return sk_inpaint.inpaint_biharmonic(img, mask_bool, channel_axis=None)
...
def segment_crystals(img, r=101, offset=25, footprint=5, remove_carbon_lacing=False, do_inpaint=True):
    offset = offset / 255.0
    img = img * (1.0 / img.max())  # float [0,1]

    mask = prediction(model, img, 256).astype(bool)

    if do_inpaint:
        img_for_seg = inpaint_with_mask_skimage(img, mask)
    else:
        img_for_seg = img

    arr = img_for_seg > filters.threshold_local(img_for_seg, r, method="mean", offset=offset)
    arr = np.invert(arr)

    # If you inpaint, usually skip this line; keep it only if grid still leaks:
    ...
    segmented = segmentation.random_walker(
        img_for_seg, markers, beta=50, spacing=(5, 5), mode="bf"
    )
